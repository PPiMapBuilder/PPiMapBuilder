...
public class UseWebServiceTask extends AbstractTask {
	...
	public void run(TaskMonitor taskMonitor) {
		...
		Map<WebServiceClient, Map> webserviceMap = webServiceHelper
				.getWebserviceMap();
		...
		while (it.hasNext()) {
			...
			// Based on the serviceLocation, we can find if the client we are
			// looking for is available
			System.out.println("Web Service Client: Display Name is "
					+ client.getDisplayName());
			System.out.println("Web Service Client: Service Location is "
					+ client.getServiceLocation());
		}
